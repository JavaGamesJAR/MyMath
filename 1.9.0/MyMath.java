...
public class MyMath
{
    ...
    public static double root(double num, double n)
    {
    	...
    	if(isInteger(n) && isEven((int)n) && num<0.0)
    	{
    		try 
    		{
				throw new ReturnsComplexException("n is even so num MUST be positive number or 0", new Throwable("n is even so num MUST be postitive number or 0"));
     ...
     * <li>if n is -&infin; returns 1.0</li>
     * </ul>
     * @param n log*(n)
     * @return log*(n)
     * @since 1.8.0
    ...
    /**Returns logariphm of f
     * 
     * @param f - number logariphm of which you get
     * @return logariphm of f
     * 
     * @since 0.5
     */
    ...
    {
    	return GaussIntegral(0, t, new DoubleFunction() {
    ...
    {
    	double result = GaussIntegral(0, t, new DoubleFunction() {
    ...
    public static double Li(double t)
    {
    	double result = GaussIntegral(2, t, new DoubleFunction() {
     ...
     * @since 0.5
     */
    public static double cosec(double f, boolean deg)
    {
     ...
     * @since 1.1
     */
    public static double excosec(double a, boolean deg)
    {
    	double result = cosec(a, deg)-1.0;
    	return result;
     ...
     * 
     * @param number - number module of which you get
     * @return
     * @since 0.5
     */
     ...
     * or sgn(x)=(|x|)'
     * 
     * @param x - number sigum of you get
     * @return signum of x
     * @since 0.9.9.25
     */
    ...
    public static double round(double number, int save)
    {
    	...
    	DecimalFormat df = new DecimalFormat(numbers);
     ...
     public static double random(double min, double max)
    {
     ...
     {
    	 int odd = random(2, (int) POSITIVE_INFINITY);
    	 odd*=2;
    	 odd-=1;
     ...
     {
    	 int even = random(1, (int) POSITIVE_INFINITY);
    	 even*=2;
    ...
    {
    	Fraction result = new Fraction(neededMethods.fractionUp(num), neededMethods.fractionDown(num));
    ...
    public static Fraction mediant(Fraction x, Fraction y)
    {
    	...
    	result = new Fraction(x.num+y.num, x.den+y.den);
    ...
    {
    	double d = (b-a)/n;
    	double sum = AM(f.execute(a), f.execute(b));
    	for(int i = 0; i < n; i++)
    	{
    		double x = a+d*i;
    		sum += f.execute(x);
    ...
    {
    	double result = 0;
    	result = AM(b, -a)*(f.execute(MyMath.AM(a, b)-(b-a)/(2*MyMath.sqrt(3)))+f.execute(AM(a, b)+(b-a)/(2*MyMath.sqrt(3))));
    ...
    public static double RiemannZeta(double s)
    {
    	double res = 1;
    	double dx = 0.001;
    	if(isInRange(-1.0+dx, 1.0-dx, s))
    	{
    		return posInf;
    		
    	}
    	else if(s>1.0 && s%2==0 && isInteger(s))
    	{
    		return minOnePow((int)s/2+1)*Bernoulli((int)s)*pow(2*pi, s)/(2*fact(s));
    	...
    	{
    		double x = 1.0-s;
    		res = 2*pow(2*pi, -x)*cos(x*pi/2.0, false)*Gamma(x)*RiemannZeta(x);
    ...
    /**Returns Pi Function (Factorial for x<0 and x&isin;C & x&isin;R
     * 
    ...
    /*public static double SternBrokSearch(double x, int LeftParentRightParent)
    {
    	double result = 0;
    	
    	double mass1[]=new double[2];
    	mass1[0]=0;
    	mass1[1]=1;
    ...
    public MyMath()
    {
    	System.out.println("This program is using MyMath library by ©JavaGamesJAR (http://fedorx.ru and https://github.com/JavaGamesJAR) this library is protected by Creative Commons Zero Universal License\nMyMath 1.7.0\nFraction 0.9\nComplex 0.9\n\n");
