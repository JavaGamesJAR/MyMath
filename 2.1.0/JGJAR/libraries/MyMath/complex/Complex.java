...
import JGJAR.libraries.MyMath.exceptions.*;
...
public class Complex extends Number
{
	...
	/**Creates new Complex from magnitude and argument
	 * 
	 * @param abs |z|
	 * @param phi Arg(z)
	 * @param nothing NOTHING!!!
	 * @since 1.2 (MyMath 2.0.0)
	 */
	public Complex(double abs, double phi, boolean nothing)
	{
		this.Re = abs*MyMath.cos(phi);
		this.Im = abs*MyMath.sin(phi);
	...
	/**Complex number = Re+i*Im, where i is a imaginary unit
	 * @since 0.5 (MyMath 1.0.0)
	 */
	public double Re;
	/**Complex number - Re+i*Im, where i is a imaginary unit
	...
	{
		return add(z1, new Complex(z2, 0));
	...
	{
		return subtract(f,new Complex(s,0));
	...
	public static Complex divide(Complex z1, Complex z2)
	{
		...
		double div = z2.Re*z2.Re+z2.Im*z2.Im; //|z_2|^2
		Complex pres = multiply(z1, new Complex(z2.Re, -z2.Im));
		return new Complex(pres.Re/div, pres.Im/div);
	...
	public static Complex sqrt(Complex z)
	{
		...
		//return root(z, TWO);
		double phi = z.Arg();
		double sqrtAbs = MyMath.root(z.Re*z.Re+z.Im*z.Im, 4);
		return new Complex(sqrtAbs*MyMath.cos(phi/2.0), sqrtAbs*MyMath.sin(phi/2.0));
	...
	{
		double phi = z.Arg();
		double cbrtAbs = MyMath.root(z.Re*z.Re+z.Im*z.Im, 6);
		return new Complex(cbrtAbs*MyMath.cos(phi/3.0), cbrtAbs*MyMath.sin(phi/3.0));
		
	}
	/**Returns log<sub>a</sub>b
	 * 
	 * @param a log<sub>b</sub>a
	 * @param b log<sub>b</sub>a
	 * @return Complex
	 * @since 0.7 (MyMath 1.1.0)
	 */
	...
	public static Complex ln(Complex z)
	{
		...
		return result;*/
		return new Complex(Math.log(z.Re*z.Re+z.Im*z.Im)/2.0, z.Arg());
	...
	{
		return MyMath.arctan2(z.Im, z.Re);
	...
	{
		double e = MyMath.exp(z.Re);
		return new Complex(e*MyMath.cos(z.Im), e*MyMath.sin(z.Im));
	...
	{
		return MyMath.sqrt(z.Re*z.Re+z.Im*z.Im);
	...
	public static Complex sin(Complex z)
	{
		...
		{*/
		return new Complex(MyMath.sin(z.Re)*MyMath.cosh(z.Im), MyMath.cos(z.Re)*MyMath.sinh(z.Im));
	...
	{
		return new Complex(MyMath.cos(z.Re)*MyMath.cosh(z.Im), MyMath.sin(z.Re)*MyMath.sinh(z.Im));
	...
	public static Complex cosh(Complex z)
	{
		...
		return divide(top, new Complex(2, 0));
	...
	/**Returns &zeta;(s) using analytic contituation
	 * 
	 * @param s argument
	 * @param n
	 * @return <i>&zeta;(s)</i>
	 * @since 1.1 (MyMath 1.9.0)
	 */
	public static Complex zeta(Complex s, int n)
	{
		Complex res = new Complex(0.0, 0.0);
		...
		if(s.Re<0)
		{
			return Complex.multiply(Complex.multiply(Complex.multiply(new Complex(-1/MyMath.PI, 0.0), Complex.multiply(factorial(new Complex(-s.Re, -s.Im), n), Complex.pow(new Complex(2*MyMath.PI), s))), Complex.sin(Complex.multiply(Complex.ONE_SECOND_OF_PI, s))), zeta(Complex.add(new Complex(-s.Re, -s.Im), new Complex(1.0, 0.0)), n));
		...
		else
		{
			for(int i = 1; i<n; i++)
			{
				res = Complex.add(res, Complex.multiply(new Complex(Math.pow(-1, i+1),0.0), Complex.pow(new Complex(i, 0.0), new Complex(-s.Re, -s.Im))));
				
			}
			res = Complex.divide(res, Complex.subtract(Complex.ONE, Complex.pow(Complex.TWO, Complex.subtract(Complex.ONE, s))));
			return res;
	 ...
	 * @since 1.0 (MyMath 1.8.0)
	 */
	public static Complex factorial(Complex z, int n)
	{
		if(isReal(z))
		{
		...
		else
		{
			...
			double sum1 = MyMath.sum(n, 1, f1);
			double sum2 = MyMath.sum(n, 1, f2);
	...
	public double doubleValue() {
		...
		{
			try {
				throw new ReturnsComplexException("Your number has imaginary part!", new Throwable("doubleValue(z) where !Complex.isReal(z)"));
	...
	public float floatValue() {
		...
		{
			try {
				throw new ReturnsComplexException("Your number has imaginary part!", new Throwable("floatValue(z) where !Complex.isReal(z)"));
			} catch (ReturnsComplexException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public int intValue() {
		...
		{
			try {
				throw new ReturnsComplexException("Your number has imaginary part!", new Throwable("intValue(z) where !Complex.isReal(z)"));
			} catch (ReturnsComplexException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
	...
	public long longValue() {
		...
		{
			try {
				throw new ReturnsComplexException("Your number has imaginary part!", new Throwable("longValue(z) where !Complex.isReal(z)"));
			} catch (ReturnsComplexException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
