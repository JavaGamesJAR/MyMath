...
import JGJAR.libraries.MyMath.Random.DefaultRandom;
import JGJAR.libraries.MyMath.Random.Dice;
import JGJAR.libraries.MyMath.Random.NormalDistribution;
import JGJAR.libraries.MyMath.Random.NormalDistribution2D;
import JGJAR.libraries.MyMath.Random.RandomGenerator;
...
public class test {
	...
	public static void main(String[] args) 
	{
		...
		*/
		...
		Dice dice = new Dice(6, new DefaultRandom());

		int ones = 0, twos = 0, thr = 0, fours = 0, fives = 0, sixs = 0;
		for(int i = 0; i<300; i++)
		{
			int random = dice.throwDice();
		...
		System.out.println("3s: "+thr);
		System.out.println("4s: "+fours);
		System.out.println("5s: "+fives);
		System.out.println("6s: "+sixs);*/
		MyMath.sgn(0);
		String qr = /*ones+""+twos+thr+fours+fives+sixs*/"t";
		byte[] bytes = qr.getBytes();
		boolean[][] bits = toBool(bytes);
	...
	{
		boolean[][] res = new boolean[array.length][8];
		for(int i = 0; i<array.length; i++)
		{
			res[i][0] = ((array[i] & 0x01) != 0);
		    res[i][1] = ((array[i] & 0x02) != 0);
		    res[i][2] = ((array[i] & 0x03) != 0);
		    res[i][3] = ((array[i] & 0x04) != 0);
		    res[i][4] = ((array[i] & 0x05) != 0);
		    res[i][5] = ((array[i] & 0x06) != 0);
		    res[i][6] = ((array[i] & 0x07) != 0);
		    res[i][7] = ((array[i] & 0x08) != 0);
			
		}
		return res;
		
	}
	public static String decode(boolean[][] a)
	{
		String res = "";
		for(int i = 0; i<a.length; i++)
		{
			String pre ="";
			for(int j = 0; j<8; j++)
				pre+=""+MyMath.IversonBracket(a[i][j]);
			char ch = (char)Integer.parseInt(pre, 2);
			System.out.println(pre+" "+Integer.parseInt(pre, 2));
			res+=ch;
